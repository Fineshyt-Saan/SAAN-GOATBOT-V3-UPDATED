...
module.exports = {
  ...
  onStart: async ({ message, event, args, usersData, api }) => {
    ...
    // ⏱️ 18 SECOND COOLDOWN
    if (!global.spinCooldown) {
      global.spinCooldown = {};
    }
    ...
    if (
      global.spinCooldown[senderID] &&
      now - global.spinCooldown[senderID] < 18000
    ) {
      return message.reply(
        "⏱️ Please wait 18 seconds before spinning again."
      );
    }

    global.spinCooldown[senderID] = now;
    ...
    if (
      ...
      now - global.spinLimit[senderID].lastReset > 3600000
    ) {
      global.spinLimit[senderID] = {
        count: 0,
        lastReset: now
    ...
    if (global.spinLimit[senderID].count >= maxSpins) {
      return message.reply(
        `🚫 Daily limit reached (${maxSpins} spins)`
      );
    }
    ...
    if (Math.random() < 0.55) {
      // WINNING SEGMENTS
      spinResult = Math.floor(Math.random() * 6) + 2;
    } else {
      // LOSING SEGMENTS
      spinResult = Math.floor(Math.random() * 2);
    }
